...
test('golden path, live popup, history, keyboard, canonical links and recovery', async ({
  ...
  await expect(dialog).not.toBeVisible();
  await expect(breaker).toBeFocused();
  // Breadcrumb to canonical Rack, then browser history back/forward.
  await page
  ...
  // Every level has a reconstructable direct URL and server-authorized login return.
  ...
  await expect(page).toHaveURL(/\/rack\/demo-rack$/);
  // Surveyed room still exposes Bay controls, independent from canvas zoom/pan.
  ...
  await page.goto(paths.get(names[4]));
